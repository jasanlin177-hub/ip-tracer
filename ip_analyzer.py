...
<style>
  ...
  /* 極簡標題：左側細線 + 近黑大標，不用色塊橫幅 */
  .kkb-hero{ border-left:3px solid #4f46e5; padding:2px 0 2px 14px; margin:.1rem 0 1.8rem; }
  .kkb-hero h1{ color:#0b0b12; font-size:1.5rem; font-weight:600; margin:0; letter-spacing:.3px; }
  .kkb-hero p{ color:#6b7280; font-size:.9rem; margin:.35rem 0 0; }
  /* 提示框：hairline、無重陰影 */
  [data-testid="stAlert"]{ border-radius:10px; box-shadow:none; border:1px solid #e6e6ec; }
  /* 指標卡：白底 hairline */
  [data-testid="stMetric"]{ background:#fff; border:1px solid #e6e6ec; border-radius:10px;
    padding:.9rem 1rem; box-shadow:none; }
  /* 側邊欄 */
  [data-testid="stSidebar"]{ background:#fff; border-right:1px solid #ececf2; }
  /* 按鈕 */
  ...
  /* 子標題近黑、克制 */
  h2,h3{ color:#0b0b12; font-weight:600; letter-spacing:.2px; }
</style>
...
def render_table(rows: list, highlight_first: bool = False) -> None:
    ...
    ths = "".join(
        f"<th style='padding:8px 12px;text-align:left;background:#f4f4f7;color:#374151;"
        f"font-weight:600;font-size:.8rem;letter-spacing:.4px;white-space:nowrap;"
        f"border-bottom:1px solid #e6e6ec'>{_html.escape(str(c))}</th>" for c in cols)
    trs = ""
    for i, r in enumerate(rows):
        bg = "background:#eef2ff;font-weight:600" if (highlight_first and i == 0) else "background:#fff"
        tds = "".join(
            f"<td style='padding:8px 12px;border-bottom:1px solid #f0f0f4;{bg}'>"
            f"{_html.escape('' if r.get(c) is None else str(r.get(c)))}</td>" for c in cols)
        trs += f"<tr>{tds}</tr>"
    st.markdown(
        f"<div style='overflow-x:auto;border-radius:10px;border:1px solid #e6e6ec'>"
